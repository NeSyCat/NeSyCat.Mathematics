...
| Categorical signatures and CD semantics (`Σ_α`, CD category, `𝓘_α`)        | stated (C3-B1: `def:categorical-signature`/`def:cd-category`/`def:categorical-interpretation` all `\leanok` in `NeSyCat/CategoricalLayer/Signatures/Signatures.lean` — `CatSignature` (three bare name fields `catSymbol`/`actorSymbol`/`monadSymbol`, `Mon := {Id, ○}`'s companion code `MonSym`, `@[blueprint_internal]`), `CDCategory` (route (a) of the LEAD encoding pin: `[Category C] [MonoidalCategory C] [SymmetricCategory C]` plus a chosen `ComonObj X`/`IsCommComonObj X` on every object, `copy_tensor`/`del_tensor` stating that the chosen comonoid on `X ⊠ Y` agrees with the one Mathlib's own `ComonObj (A ⊗ B)` instance induces from `X`'s and `Y`'s — no law beyond what the environment states), and `CatInterpretation` (`cd : CDCategory`, an actor category `A`, the actegory action `act : A → cd.C → cd.C` as a bare object map with NO functoriality/coherence fields — the environment states none, and `def:domain-interpretation` only ever applies it to objects — and `monad : CategoryTheory.Monad cd.C`; `CatInterpretation.interpretMon`, `@[blueprint_internal]`, is the companion definitional clause for "`Id` is always interpreted by the identity functor"). All three definitions only, no theorem proofs in this batch; the three running interpretations (Set/Tens/Identity, Set/Tens/Dist, Tens/Tens/LogVec) are prose examples, not envs, and are not formalized. Gates: `scripts/check.sh` GREEN, `scripts/sorry-report.sh` 0/0, `scripts/blueprint.sh` GREEN (structure 88 environments unchanged, kind-check 56 -> 59 names, kernel-truth OK 59, census 317/56/261/0 -> 333/59/274/0 -- the 16 new scanned declarations are the 3 cited structures plus `MonSym`/`CatInterpretation.interpretMon` and 11 `deriving`-synthesized `DecidableEq`/`Repr` byproducts, all `@[blueprint_internal]`, matching the `BoolW` precedent for post-hoc-tagged derived instances -- 0 unclassified, registry sync 9 twins, structure-mirror 15/0), pdf+web rebuilt clean (0 overfull hboxes).) C4-ERR (2026-08-12): ERRATUM REPAIRED in `def:cd-category`. `CDCategory.copy_tensor`/`del_tensor` ELABORATED TO REFLEXIVITY and had done so since the field was written: `comon : ∀ X : C, ComonObj X` is itself in scope as an instance inside the structure declaration, so each field's `ComonObj.comul (X := X ⊗ Y)` re-synthesized the CHOSEN comonoid on the tensor instead of Mathlib's induced one, making both sides the same term. `cd.del_tensor X Y` was a proof of `x = x`. Repaired by writing the induced side out explicitly via Mathlib's own `Comon_.tensorObj_comul`/`tensorObj_counit` formulas, which no instance search can redirect. REFUTABILITY CHECKED BEFORE LANDING (the check whose absence let this survive): `rfl` no longer closes either field for an arbitrary `comon` family, where before the field WAS reflexivity. `typeCD` needed NO change — its proof already went through Mathlib's genuine `CopyDiscardCategory` laws, so it now discharges a real obligation where it had discharged an empty one; whole-project build green, confirming the blast-radius assessment (one consumer, no dependents, because nothing can be built on a vacuous law). `InterpretationMorphism`'s `comul_tensor`/`counit_tensor`, added as the workaround while the defect stood, are now redundant rather than load-bearing; folding them back is left as tidying and is recorded as such in the blueprint prose. THE LESSON, worth more than the bug: every gate in this repo passed the vacuous version, and V-C4-T1 explicitly checked that typeCD "genuinely discharges copy_tensor/del_tensor, not sidestepped" and PASSED it — that check confirmed the PROOF did work and never asked whether the GOAL said anything. A non-trivial proof of a vacuous statement is still a non-trivial proof. Law fields must be checked for REFUTABILITY, not merely for being proved. Gates: CHECK GREEN 0 errors, 0/0, BLUEPRINT GREEN 127 envs / 125 names, census 746/125/621/0, all sentinels unchanged. C4-ERR follow-up (2026-08-12, user ruling "we should remove that redundancy"): the `comul_tensor`/`counit_tensor` fields `InterpretationMorphism` carried while the CDCategory laws were vacuous are REMOVED, and their two use sites re-pointed at `I.cd.copy_tensor`/`I.cd.del_tensor` — the statements were character-identical, so the swap was direct and the file rebuilt clean with no proof changes. One statement of a law, in the place that owns it; two copies of a law invite drift. The vacuity gate's own count moved 41 -> 39 Eq-shaped law fields, exactly the two removed, which is the removal confirming itself. |
...
| Grammatical layer — C3-B4-FIX (verification-driven repair of the Kleisli interpretation) | proved (blind verifier confirmed `def:kleisli-interpretation`'s overall encoding faithful except five gaps, all now repaired in `NeSyCat/GrammaticalLayer/{Grammar,Kleisli}.lean`: (1) CONTEXT FIDELITY — `Fm.on` now returns the genuinely deduplicated context (`firstDedup`, an order-preserving first-occurrence dedup, `Grammar.lean`, plus its `mem_firstDedup`/`firstDedup_nodup` companions) for the `.rel`/`.conn` clauses, matching `def:context`'s distinct-list reading; `Fm.sem`'s `.rel`/`.conn` clauses now prepend the new `ctxCopy` companion (`Kleisli.lean`: `ctxProjFilter`/`filter_eq_singleton_of_nodup_mem`/`projTo` build the single-variable projection, `ctxCopy` itself comultiplies the whole deduplicated context once per target-list position and projects), routing one copy of each shared variable to every raw occurrence — exactly the document's own `copy : I([φ⃗]) -> I([φ_1]) ⊠ ... ⊠ I([φ_n])`; (2) POSITIONAL SUBSTITUTION — `Fm.on`'s `.subst` clause now splices `t.inn` at `x`'s own position in `body.on` (via `List.takeWhile`/`List.dropWhile`, plus the new `list_split_pre_post` reassembly lemma needing only `x ∈ body.on`) rather than appending it at the list's end, matching the document's `[φ]_{[x]↦inn(ξ)} = [y_1,...,y_{p-1}] + inn(ξ) + [y_{p+1},...,y_m]` reading; `Fm.sem`'s `.subst` clause rebuilt to match, threading the term's (unitor-squeezed) value through `SI.leftStrength`/`SI.strength` into the reassembled position, joined via `SI.bind`; (3) BOTH MARKERS — `Fm.KTyped`'s `connMonad c = ○`/`quanMonad Q = ○` restrictions dropped; `Fm.sem`'s `.conn`/`.quant` clauses now dispatch on `sigB.connMonad c`/`sigB.quanMonad Q` directly, composing `FmList.sem`'s tensor of monadic factors straight into `connMor`/`quanMor` at the `○`-marked instance, or first collapsing it via the new `dstFoldN` (`n`-ary `StrongCatInterpretation.dst`-fold, `Kleisli.lean`) before applying the plain `Id`-marked morphism inside the monad — the `def:wire-adapters` `○`/`Id` bind-set row, rebuilt directly from `dst` rather than by invoking `feedForward`; (4) the substitution clause's `Fm.KTyped` conjunct is now literally the document's own `x ∈ body.on` (the exactly-once filter condition dropped — with `body.on` a genuine context whenever it is itself dedup'd/filtered, mere membership already forces uniqueness, and `Fm.sem`'s new positional construction only ever needs `x`'s first occurrence regardless); (5) `Kleisli.lean`'s module doc rewritten to record the file's own remaining deviations in place (no `feedForward` reuse — this file rebuilds `⨟`-composites directly from `SI.dst`/`strength`/`bind`; the `funMorK`/`relMorK` parameter-free bridging, no-coherence debt; the `varCard`/`varPt` finite-enumeration-completeness encoding), and the PROGRESS.md C3-B3 entry's false `feedForward`-reuse claim corrected in place. Gates: `scripts/check.sh` GREEN (whole project), `scripts/sorry-report.sh` 0/0, `scripts/blueprint.sh` GREEN (structure 88 environments unchanged, kind-check 69 names unchanged — no cited name renamed, `content.tex` untouched, kernel-truth OK 69, census 406/69/337/0 -> 415/69/346/0 — the 9 new scanned declarations are all `@[blueprint_internal]` companions/byproducts (`firstDedup`, `mem_firstDedup`, `firstDedup_nodup`, `list_split_pre_post` in `Grammar.lean`; `ctxProjFilter`, `filter_eq_singleton_of_nodup_mem`, `projTo`, `ctxCopy`, `dstFoldN` in `Kleisli.lean`) — 0 unclassified, registry sync 9 twins, structure-mirror 15/0), pdf+web rebuilt clean (0 overfull hboxes, `content.tex` unchanged so nothing to rebuild against). C4-T2a CHECKPOINT (partial, honestly): NEW `NeSyCat/GrammaticalLayer/InterpretationMorphism.lean` + `def:interpretation-morphism` (`NeSyCat.InterpretationMorphism`, \leanok) — what it takes to carry one Kleisli interpretation to another over the same CD category. The bundle carries an OBJECT-level family `domMor` AND a `CategoryTheory.MonadHom` `monadMor` (plus `omegaMor`), not the monad morphism alone: `thm:pointwise-eval-kleisli` batches its INPUT (`s : Bidx → I(S)`) as well as its output, so the two readings differ in objects too, and a monad-only bundle could not discharge the env. Laws travel as HYPOTHESES, never as new fields on any existing record — `strength_natural` is a law of the target interpretation alone and sits here precisely so that no already-formalized item acquires an obligation (same pattern as `batchTransformer`'s `hleft`). ZERO BLAST RADIUS: nothing existing modified. 11 `@[blueprint_internal]` companions (`withMonad`, `markerMor`, `interpretMSMor`, `interpretPowMor`, `ctxObjMor`, `msMor`, `ctxMor`, the two strength naturality lemmas, `leftStrength_compat`, `dst_compat`). WHAT IS NOT DONE: the induction itself. Neither the term half (`Tm.sem`/`TmList.sem`) nor the formula half (`Fm.sem`/`FmList.sem`) is proved, and `thm:pointwise-eval-kleisli` remains UNMARKED. This landed as a checkpoint because the frontier-seat worker died mid-run (machine sleep, the fourth such death in the campaign) with a compiling file and no commit; the design work was worth preserving rather than re-deriving. Gates: CHECK GREEN 0 errors, 0/0, BLUEPRINT GREEN 126 envs / 124 names, census 681/124/557/0 (+1 cited = the structure, +11 internal = its companions), all other sentinels unchanged. C4-T2a-cont (THE TERM HALF, proved): NEW `lem:kleisli-term-natural` `\leanok` on both statement and proof, cited principal `NeSyCat.Tm.sem_natural`, appended to `NeSyCat/GrammaticalLayer/InterpretationMorphism.lean` with nothing existing modified (append-only, zero blast radius kept). The statement, verbatim in the document's notation: for a morphism of interpretations with monad morphism $\theta$, and every term $\xi$ satisfying `def:kleisli-interpretation`'s typing side condition, $\sem\xi^1 \seq \theta \seq \mathcal M_2\Phi_{\mathrm{val}(\xi)} = \Phi_{\inn(\xi)} \seq \sem\xi^2$. Proved by mutual structural induction over `Tm` and `List Tm`, with `TmList.sem_natural` (`@[blueprint_internal]`) the list half: variable clause = `MonadHom.app_η` then naturality of the target unit; functional clause = naturality of $\theta$, `funMorK_compat`, then the list hypothesis; nil = the unit law again; cons = `dst_compat`, the two concatenation-iso splittings (`msMor_kcodAppendIso_inv`/`ctxMor_ctxAppendIso`, each an induction on the first list whose base case is left-unitor naturality and whose step is associator naturality), and `StrongCatInterpretation.dst_natural` fed by `Φ.strength_natural`. THE HAZARD AND WHAT IT COST, recorded because the ticket flagged it as the stop condition: `Tm.inn` and `Tm.KTyped` are compiled by recursion over a NESTED inductive and do NOT reduce definitionally (checked directly -- `(Tm.var x).inn = [x]` is not closed by `rfl`), so `Tm.sem`'s generated equations carry `Eq.mpr`/`Eq.rec` casts, exactly as the ticket predicted. The stop condition was NOT reached and no cited principal was rewritten: the casts are converted ONCE into `eqToHom` composites (`Tm.sem_var_eq`/`Tm.sem_app_eq`, both proved through `HEq` -- `cast_heq`/`eqRec_cod` then `eqToHom_comp_heq` -- rather than by matching the cast's own proof term), and the residual friction, a rewrite motive that is ill-typed at `rw`'s DEFAULT `instances` transparency (`η.app X` has domain `(𝟭 C).obj X`, and `Tm.sem`'s codomain is `interpretMS I D.domObj ξ.kcod` where the clause lemma's is `ctxObj D [x]`), is cleared by `rw (config := { transparency := .default })` throughout, which changes no statement. 13 new `@[blueprint_internal]` companions (`eqRec_cod`, `Tm.inn_var`, `Tm.inn_app`, `Tm.ktyped_app`, `Tm.sem_var_eq`, `Tm.sem_app_eq`, `msMor_eqToHom` + its `reassoc` twin, `ctxMor_eqToHom` + its twin, `funMorK_comp_msMor`, `msMor_kcodAppendIso`, `msMor_kcodAppendIso_inv`, `ctxMor_ctxAppendIso`, `TmList.sem_natural`) plus post-hoc tags on four `Tm.sem`/`TmList.sem` equation/congruence byproducts this module forces into the environment. `eqRec_cod` is the auxiliary transport lemma of general interest the ticket anticipated: it is stated for an arbitrary family `F : α → C` of objects, which is all its proof uses. WHICH PRE-BUILT COMPANIONS EARNED THEIR KEEP: `msMor` and `ctxMor` (the statement is written in them), `dst_compat` and `StrongCatInterpretation.dst_natural` (the cons clause), `leftStrength_compat` and `leftStrength_natural` (through those two), `withMonad`/`markerMor`/`interpretMSMor`/`ctxObjMor` (throughout); `interpretPowMor`, `omegaMor`, `relMorK_compat`, `connMor_compat` and `quanMor_compat` are untouched, being the formula half's data. STEP CORRESPONDENCE, one finding worth recording: a `mutual` block compiles each of its theorems to a proof term mentioning only the compiler's own bundle `<name>._f`, so the C3-ISAR step scan's closure (which descends only through `@[blueprint_internal]` constants) resolved NONE of the item's tags until `Tm.sem_natural._f`/`TmList.sem_natural._f` were themselves tagged; with those two tags the item is phase-covered and all three of its displays resolve. The `have`-naming house rule is followed for the steps a display cites (`list_natural`, `head_natural`, `tail_natural`, never `ih`). STILL NOT DONE, unchanged: the formula half (`Fm.sem`/`FmList.sem`) and the instantiation at `ev i`; `thm:pointwise-eval-kleisli` remains UNMARKED. Gates: `scripts/check.sh` GREEN (whole project, no new warning), `scripts/sorry-report.sh` 0/0, `scripts/blueprint.sh` GREEN -- structure 126 -> 127 environments (the one new `lemma` env), kind-check 124 -> 125 names, kernel-truth OK 125 (`propext`/`Classical.choice`/`Quot.sound` only), census 681/124/557/0 -> 701/125/576/0 (the 20 new scanned declarations are the 1 cited name plus 19 internals), classification sentinel total=61 -> 62 arithmetic=56 -> 57 structural=5 unchanged (the new item is carrier-algebraic, via `CategoryTheory`) with the rendered dichotomy table's counts moved with it, step-correspondence sentinel envs=5 -> 6 displays=15 -> 18 tags=15 -> 18 names=55 -> 63, registry 9, structure-mirror 17/0; pdf+web rebuilt clean (0 overfull hboxes). C4-T2b (THE FORMULA HALF, PARTIAL -- the induction itself is NOT landed; three self-consistent, gate-green increments toward it, all append-only, nothing pre-existing modified). (1) `Fm.sem`'s four clauses read off cast-free, the formula-side twins of `Tm.sem_var_eq`/`Tm.sem_app_eq`: `Fm.on_rel`/`on_conn`/`on_quant`/`on_subst` and `Fm.ktyped_rel`/`ktyped_conn`/`ktyped_quant`/`ktyped_subst` release the two nested-inductive recursions, and `Fm.sem_rel_eq`/`sem_conn_eq`/`sem_quant_eq`/`sem_subst_eq` state each clause as an `eqToHom` composite, proved through `HEq` as the term half's were. TWO CAST SHAPES THE TERM HALF NEVER MET: a `Prop` structure has no definitional eta, so a clause's `obtain` on its typing witness survives as a blocked `And.rec` (`and_rec_proj` releases it to that witness's own projections, proof irrelevance then identifying them with any other witness); and a clause that rewrites a hypothesis rather than the goal leaves a `cast` in a morphism's codomain rather than an `Eq.rec` (`cast_hom_cod` is `eqRec_cod`'s twin). `connMorAt`/`quanMorAt` name the marker-dispatched connective/quantifier morphisms. (2) The copy machinery carried across a morphism of interpretations: `ctxMor_comul`/`ctxMor_counit` (the induced morphism on a context's tensor is a comonoid morphism), then `ctxProjFilter_cons_pos`/`ctxProjFilter_cons_neg` (that definition's two head clauses cast-free), `ctxProjFilter_compat`, `projTo_compat`, `ctxCopy_compat`. FOUR NEW FIELDS on `InterpretationMorphism`, all dischargeable in the batch instance: `domMor_comul`/`domMor_counit` (every component of the object-level family is a comonoid morphism; `ev i` is one, the CD structure on types being cartesian) and `comul_tensor`/`counit_tensor`. ERRATUM, disclosed in `content.tex` after `def:interpretation-morphism` (prose) and as a `%` comment there: `CDCategory.copy_tensor` and `CDCategory.del_tensor` already claim exactly the two tensor-compatibility laws but elaborate to REFLEXIVITY. Inside the structure declaration the field `comon : forall X : C, ComonObj X` is itself a local instance, so each field's `ComonObj.comul (X := X (x) Y)` synthesizes `comon (X (x) Y)` rather than Mathlib's induced tensor comonoid. Confirmed by `#check @CDCategory.del_tensor` under `pp.explicit` and by a negative control discharging `(cd.comon (X (x) Y)).counit = (cd.comon (X (x) Y)).counit` with `cd.del_tensor X Y`. Repairing the two fields means editing a pre-existing record, outside C4-T2b's write set, so the laws travel on the new structure and the defect is written down. NEEDS ITS OWN TICKET. (3) The `Id`-marked marker branch discharged, the branch long reported unprovable because `StrongCatInterpretation` carries the strength as bare data: `dstFoldN_compat` spends `strength_compat` and `strength_natural` (through `dst_compat` and `dst_natural`) to move the `n`-ary strength collapse across the two readings. `markerAdapt` writes the dispatch with the marker as an ordinary argument rather than as a dependent match's scrutinee, so a case analysis on it produces no transports; `markerAdapt_compat` proves the compatibility once for both markers and both symbol kinds; `connMorAt_eq`/`quanMorAt_eq` (through `markerAdapt_cast`) identify `Fm.sem`'s own dependent match with it, and `connMorAt_compat`/`quanMorAt_compat` read off `connMor_compat`/`quanMor_compat`. WHAT REMAINS for the induction, enumerated: the list half's own induced morphism on `tensorList (fs.map fun _ => M(Omega))` and its bridge to `interpretPowMor ... MonSym.mon fs.length` across `List.map_const'`; the mutual `Fm.sem_natural`/`FmList.sem_natural` block itself; and, for the quantified clause only, `comulN_compat`, `tensorFin_compat`, cast-free clause lemmas for `ctxMerge` plus its compat, and `listCard`/`listPt` compat, which need two further fields (`varCard_compat`, `varPt_compat`) since `KleisliInterpretation` carries the product-state enumeration as data of each reading. `thm:pointwise-eval-kleisli` remains UNMARKED. Gates at each of the three commits: `scripts/check.sh` GREEN (whole project), `scripts/sorry-report.sh` 0/0, `scripts/blueprint.sh` CORRESPONDENCE: OK then BLUEPRINT: GREEN -- 127 environments and 125 names throughout (no new env, no new cited name), census 701/125/576/0 -> 722/125/592/0 -> 734/125/609/0 -> 746/125/621/0 (every new declaration an `@[blueprint_internal]` companion, 0 unclassified), classification total=62 structural=5 arithmetic=57, step-correspondence envs=6 displays=18 tags=18 names=63, registry 9, structure-mirror 17/0. C4-T2b-cont (2026-08-12), THE FORMULA HALF IS PROVED: `lem:kleisli-formula-natural` is a NEW `\lean{NeSyCat.Fm.sem_natural}\leanok` lemma env, and with it the naturality induction of `def:interpretation-morphism` is complete on both halves. Three commits, one per increment the previous seat enumerated. (1) `fmListMor`, the morphism induced on `FmList.sem`'s tensored codomain (every slot `○`-marked), and `fmListMor_map_const`, its bridge to `interpretPowMor ... MonSym.mon fs.length` across `List.map_const'`, out of `eqToHom_tensorList_cons` (a list transport splits over a `tensorList` cons). The bridge's own statement is type-correct only at default transparency, one side's codomain being a `tensorList` of a `replicate` and the other's an `interpretPow` of the same data, so every step of its proof is a default-transparency rewrite. (2) The quantified clause's machinery: `comulN_compat`, `tensorFin_compat`, `ctxMerge_cons_pos`/`ctxMerge_cons_neg` (cast-free, via the NEW `eqRec_dom`, `eqRec_cod`'s twin for a DOMAIN transport) with `ctxMerge_compat`, `quantState_compat` (per-state insertion) and `quantSum_compat` (the whole copy-read-quantify composite). TWO NEW FIELDS on `InterpretationMorphism`, both refutable (the vacuity gate moved 39 -> 41 Eq-shaped law fields, exactly the two): `varCard_compat` (the two readings enumerate a variable's domain to the same length) and `varPt_compat` (the family carries the i-th point to the i-th point). Without the first, `listCard K1 l` and `listCard K2 l` are different naturals and the two readings of a quantified formula share no arity at all; `quantSum_compat` takes both arities and their equation as ordinary variables, so one `subst` identifies them, and `listCard_compat`/`listPt_compat` are stated against the underlying index rather than a transport of it. `comulN`/`tensorFin` on the target side must be written at `(I := I.withMonad M2)` explicitly: `@comulN sigA I X n` and `@comulN sigA (I.withMonad M2) X n` are NOT definitionally equal at a variable arity, and `rfl` says so. (3) The four clauses as standalone lemmas (`Fm.rel_natural`, `Fm.conn_natural`, `Fm.quant_natural`, `Fm.subst_natural`, each taking the induction hypotheses it needs as hypotheses) plus the mutual `Fm.sem_natural`/`FmList.sem_natural` block, whose four cases are then one `exact` each; `attribute [blueprint_internal] Fm.sem_natural._f FmList.sem_natural._f` so the step gate's closure resolves the tags. The substituted clause is the long one and factors through `term_slot` (the term half squeezed by the unitor onto the substituted variable's own object, via `msMor_singleton_id` and `rightUnitor_naturality`) and `left_slot` (both strengths carried along the monad morphism by `leftStrength_compat` and natural in the target); `bind_compat` (NEW) carries `bind` across the monad morphism out of its own multiplication law. `def:interpretation-morphism`'s prose is corrected in the same pass on a second count: it still listed the two tensor compatibilities as its own laws, which they stopped being at C4-ERR, and the paragraph that counted three reading-independent laws counted them too. `thm:pointwise-eval-kleisli` remains UNMARKED: instantiating at `ev i` is the next ticket. Gates at each of the three commits: `scripts/check.sh` GREEN, `scripts/sorry-report.sh` 0/0, `scripts/blueprint.sh` CORRESPONDENCE: OK then BLUEPRINT: GREEN; 127 envs / 125 names -> 128 envs / 126 names (the one new lemma env), census 746/125/621/0 -> 749/125/624/0 -> 763/125/638/0 -> 774/126/648/0 (0 unclassified throughout), vacuity 39/0 -> 41/0, classification total=62 -> 63 with structural=5 unchanged, step-correspondence 6/18/18/63 -> 7/23/23/82 (`lem:kleisli-formula-natural` joins the phase list with five tagged displays), registry 9, structure-mirror 17/0. Axiom audit on `NeSyCat.Fm.sem_natural`: propext, Classical.choice, Quot.sound only. |
| Batch monad + transformer + pointwise evaluation                           | partial (C3-B6(+B10): `def:batch-monad`/`thm:batch-transformer` `\leanok` in `NeSyCat/StatisticalLayer/Batching/{BatchMonad,BatchTransformer}.lean`; `thm:pointwise-eval` PART 1 ONLY `\leanok`, part 2 split out honestly. `Bmon B X := Fin B → X` (PLAIN FUNCTIONS, disclosed encoding pin: `Fin B`-indexed reader monad, hand-rolled `Bmon.ret`/`Bmon.bind` mirroring `SemiringMonad.lean`'s own `ret`/`bind` style — no `Monad`/`LawfulMonad` instance for `MS S` exists to route through Lean's `ReaderT (Fin B) Id` here). `thm:batch-transformer` switches encodings once an inner effect monad `M` enters: `BmonT B M X := ReaderT (Fin B) M X` (Lean/Mathlib's own `ReaderT`, disclosed), reusing Lean core's `LawfulMonad (ReaderT ρ m)` instance (`Init/Control/Lawful/Instances.lean`, given `[Monad M] [LawfulMonad M]`) as BACKGROUND machinery for "the composite is again a monad" clause, and proving the two canonical lifts by hand: `liftM` (the `MonadLift`-shaped embedding, definitionally Lean core's own `ReaderT` `MonadLift` instance) and `liftBmon` (a `MonadFunctor`-SHAPED lift along `M`'s unit, DISCLOSED as not literally an instance of Lean core's `MonadFunctor` class — that class's `monadMap` signature is a same-monad endo-map `{β} → m β → m β`, not a monad-changing map `Id ⟶ M`, so `liftBmon`/`liftBmon_ret`/`liftBmon_bind` are hand-proved, the bind clause reducing to `M`'s own left-unit law `pure_bind`). Bundled principal `batchTransformer` cites `liftM_ret`/`liftM_bind`/`liftBmon_ret`/`liftBmon_bind` (all `@[blueprint_internal]` companions), matching `semiring_monad_laws`'s own bundling precedent. `thm:pointwise-eval` SPLIT (bijection-law peer-claim split, two genuine claims the original env bundled): PART 1 keeps the `thm:pointwise-eval` label (`ev_i` is a monad morphism, unconditional — `NeSyCat.ev_isMonadMorphism`, `\leanok` both sides, two `rfl`s from `ReaderT`'s own diagonal-bind unfolding); PART 2 is the NEW env `thm:pointwise-eval-kleisli` (commutation with the Kleisli interpretation of `def:kleisli-interpretation` under the batch-naturality hypothesis) — carries NO `\lean`/`\leanok` mark on either statement or proof, left for a rider ticket: it needs a generic account of the Kleisli interpretation over an ARBITRARY strong monad `M` (to instantiate at both `M` and `Bmon M`), which `NeSyCat/GrammaticalLayer/Kleisli.lean` does not provide (built only for the library's own concrete interpretations) — the real dependency, not literally `lem:quantifier-nestable`/B4b (a different open item) though the spec named that as the blocking rider. B10 stray `lem:bind-matrix-mult` `\leanok`: retagged the PRE-EXISTING `NeSyCat.bind_apply` (`SemiringMonad.lean`, formerly an undisclosed `@[blueprint_internal]` companion of `def:semiring-monad`'s `bind`) as this lemma's own cited principal — its formula `bind f k y = f.sum fun x w => w * k x y` already IS the blueprint's matrix-multiplication identity, immediate unfolding, no new Lean needed. Gates: `scripts/check.sh` GREEN (whole project), `scripts/sorry-report.sh` 0/0, `scripts/blueprint.sh` GREEN (structure 88 -> 89 environments — the one new `thm:pointwise-eval-kleisli` env, kind-check 69 -> 73 names — `NeSyCat.Bmon.bind`/`NeSyCat.batchTransformer`/`NeSyCat.ev_isMonadMorphism`/`NeSyCat.bind_apply`, kernel-truth OK 73, census 415/69/346/0 -> 428/73/355/0 — the 13 new scanned declarations are the 3 newly-cited names (`bind_apply` was already scanned, now flips internal -> cited) plus 10 `@[blueprint_internal]` companions/byproducts (`Bmon`, `Bmon.ret`, `BmonT`, `liftM`, `liftBmon`, `liftM_ret`, `liftM_bind`, `liftBmon_ret`, `liftBmon_bind`, `ev`) — 0 unclassified, registry sync 9 twins, structure-mirror 15/0), pdf+web rebuilt clean (0 overfull hboxes).) C3-EXEC item 1 (referee response, carrier-agnostic exactness): NEW `thm:batch-exactness` `\leanok` in `NeSyCat/StatisticalLayer/Batching/BatchTransformer.lean` (`NeSyCat.batch_layer_exact`, a six-clause bundling theorem citing `batchTransformer`/`ev_isMonadMorphism` verbatim, no new proof) — the point being that its OWN statement is fully generic (`X Y : Type` unconstrained — `Type`, not `Type*`, the inner monad being taken at `Type -> Type*`; corrected at C3-EXEC-FIX — with `[Monad M]` only and NO `[LawfulMonad M]` since C3-EXEC-FIX2, no `Semiring`/`LatCSRng`/lattice hypothesis anywhere), mechanically confirmed by the classification gate (item 2, corrected and gated at C3-EXEC-FIX) finding zero algebraic markers in its type. Prose after the env states the a-fortiori float-carrier consequence and the scope disclaimer (batching contributes no error of its own; this does not claim float arithmetic is exact), plus the explicit non-goals paragraph (IEEE-754/float-bound work stays open, `thm:chain-bound` cited as the template for that bound's shape). C4-T1 (the concrete instantiation substrate at `Type`): landed the five-item substrate a later ticket needs to prove `thm:pointwise-eval-kleisli`, all as real library code plus new blueprint items, none of them marking `thm:pointwise-eval-kleisli` itself (still `partial`, unchanged). NEW `NeSyCat/StatisticalLayer/Batching/TypeInstantiation.lean`: `def:type-cd-category` (`typeCD`, `Type u` as a `CDCategory` via Mathlib's `CartesianCopyDiscard` bridge, universe-polymorphic, specialized to `Type 0` downstream), `def:type-strength`/`lem:type-strength-laws` (`typeStrength`/`typeStrength_naturality_left` cited, the other five of the six strength laws — `_naturality_right`/`_unit`/`_assoc`/`_left_unitor`/`_assoc_coherence` — `@[blueprint_internal]` companions of the one principal, the `otimesM_assoc`/`lem:linear-lift` bijection-law precedent), `def:type-cat-interpretation` (`typeCatInterpretation`, a `CatInterpretation` at `typeCD` with `monad := CategoryTheory.ofTypeMonad M` for a lawful `M : Type → Type`, actor category taken as `Type` itself acting by its own product — disclosed simplest choice, the action carries no coherence law to select among), `def:ev-monad-hom` (`evMonadHom`, `ev i` read as a `CategoryTheory.MonadHom` between the two `ofTypeMonad` realizations, `app_η`/`app_μ` matching `ev_isMonadMorphism`'s two clauses exactly, naturality via `ReaderT.run_map`), `lem:ev-strength-natural` (`ev_strength_natural`, `ev` commutes with the canonical strength across `M` and `BmonT B M`, depends on NO axioms at all, `LawfulMonad` unused, closes via `ReaderT.run_bind` then `rfl`). UNIVERSE RESTRICTION, disclosed both in the Lean module doc and in blueprint prose: `ofTypeMonad` needs `Type u → Type u`, and `Fin B : Type 0` pins `u = 0` for the whole composite `BmonT B M` to land back in `Type → Type`, so this substrate covers `M : Type → Type` only. `MS S` does NOT satisfy this: it carries no Lean `Monad`/`LawfulMonad` INSTANCE anywhere in the library (`bind`/`ret` are bare functions, per `SemiringMonad.lean`'s own module doc), so `ofTypeMonad (MS S)` does not typecheck as written — registering that instance is out of this ticket's scope, left for a future ticket. `Idmon` DOES satisfy it: it has no separate Lean object (the identity case is definitionally trivial, per `TruthSpace.lean`'s own doc), and read as Lean core's `Id : Type u → Type u` it is a lawful `Type → Type` monad at every universe. The batch-layer dichotomy table/prose (before `thm:pointwise-eval-kleisli`) is updated to fold in the two new carrier-agnostic theorem-kind items (`typeStrength_naturality_left`, `ev_strength_natural`), landing at 5 of 61 carrier-agnostic rather than 3 of 59. Gates: `scripts/check.sh` GREEN (whole project), `scripts/sorry-report.sh` 0/0, `scripts/blueprint.sh` GREEN — structure 119 -> 125 environments (the 6 new envs), kind-check 117 -> 123 names, kernel-truth OK 123 (only `propext`/`Classical.choice`/`Quot.sound`; `ev_strength_natural` and `typeStrength` themselves depend on NO axioms at all), census 667 scanned (123 cited, 544 internal, 0 unclassified — the 11 new declarations are the 6 cited names plus 5 `@[blueprint_internal]` companions), classification sentinel total=59->61 structural=3->5 arithmetic=56 unchanged (both new structural names verified via the mechanical `getUsedConstants` scan before writing the sentinel line, not guessed), step-correspondence sentinel unaffected by the new items (still phase-gated to the five pre-existing labels) but its own `names` count dropped 55->54 as an R1 side-repair (below), registry sync 9 twins, structure-mirror 17/0 unchanged (same folder, no new tag needed). Two riders from the C3-ISAR P1 verification, batched into this same commit: R1, `content.tex`'s `lem:tilt` proof `% lean-step:` tag over-attributing `reweighted_sum` to a display that only transcribes `tilt_mass`'s content (the display shows the denominator/mass computation only; `reweighted_sum`'s numerator identity, with its extra `dec(k x) y` factor, appears only in the prose sentence before the display, not in the display itself) — fixed by dropping `reweighted_sum` from the tag (`tilt_mass, NeSyCat.dec_mul_Z` remain, both genuinely reachable from that display), which required a matching `% LEAN-STEP-SENTINEL:` names count update (55 -> 54, the only number that moved). R2, added one sentence to FORMALIZE.md's step-tag convention (item 2, "The gate") documenting that a display legitimately transcribing a `@[blueprint_internal]` companion's own proof body (reached through the gate's documented transitive closure), rather than the cited principal's literal one-line delegating body, is the intended reading of "the cited declaration's own proof," not a defect — this is `lem:tilt`'s own recurring shape (`tilt_bind` delegates in one line to `tilt_bind'`, where the real mathematics lives), and it recurred three times in the C3-ISAR P1 batch (`dec_bind_of_massPreserving`/`tilt_bind'`, `bind_layer_sandwich`) without a name for the pattern until now. DISCLOSED CHOICES (not specified by the ticket, judgment calls made while landing this): (1) the actor category for `def:type-cat-interpretation` is `Type` acting on itself by its own product (`act := fun a x => a × x`), the simplest available witness since `def:categorical-interpretation`'s `act` field carries no coherence law to select among — NOT literally the `\mathbf{Tens}` actor of the earlier-named "Set/Tens/Identity" running example, disclosed as such in both the Lean doc comment and the blueprint prose; (2) the six strength laws are landed as six separate Lean theorems (matching the lead's description) but ONE blueprint env, following the `lem:linear-lift`/`otimesM_assoc` precedent of citing a single principal name and tagging the rest `@[blueprint_internal]`, rather than bundling all six into one fresh conjunction theorem the way `semiring_monad_laws` bundles the three monad laws; (3) `evMonadHom`/`ev_strength_natural`'s tactic proofs needed several `ConcreteCategory`/`TypeCat.Hom` bundling workarounds (`ConcreteCategory.hom_ext`, `set` to retype an awkward `Functor.comp`-composed hypothesis type before feeding it to `ev_isMonadMorphism`, explicit named-argument instantiation of `ev_isMonadMorphism`'s implicit `X`/`Y`/`B`/`M` to work around a `rw`-internal transparency restriction) — exactly the "boilerplate, not mathematics" friction the ticket anticipated, disclosed here as engineering, not a deviation from the mathematical content. C4-T1-FIX (batched remediation of a blind-verifier FAIL on C4-T1, four attribution/disclosure defects, no change to any statement or proof term): F1, the five cited principals in `TypeInstantiation.lean` (`typeCD`, `typeStrength`, `typeCatInterpretation`, `evMonadHom`, `ev_strength_natural`) had doc comments opening "Toward `thm:pointwise-eval-kleisli`: ..." without citing their own blueprint label, violating FORMALIZE.md's mandated `/-- Blueprint <label> (<name>): ... -/` form (that phrasing is correct only on the five `@[blueprint_internal]` companions, left untouched); all five rewritten to the mandated form, the "toward" motivation kept as a trailing sentence. F2, `lem:tilt`'s proof cited `NeSyCat.dec_mul_Z` on its denominator display (`tilt_mass, NeSyCat.dec_mul_Z`), but `dec_mul_Z` is used only inside `reweighted_sum`'s numerator cancellation (`Tilt.lean:244-249`), which the prose narrated but never displayed; fixed the right way (R1's other sanctioned option) by adding the missing numerator display, tagged `% lean-step: reweighted_sum, NeSyCat.dec_mul_Z`, and leaving the denominator display tagged `tilt_mass` alone — both names now genuinely transcribed, `% LEAN-STEP-SENTINEL` updated to `envs=5 displays=15 tags=15 names=55` (displays/tags each +1, names 54->55), all three numbers read off `scripts/blueprint.sh`'s own step-scan output, not guessed. F3, the C4-T1 entry above claims the `MS S` non-coverage consequence is "disclosed both in the Lean module doc and in blueprint prose"; true of the Lean doc (`TypeInstantiation.lean:33-47`) but NOT of `content.tex` as landed — the motivating paragraph before `def:type-cd-category` stated only the universe restriction, never that `MS S` itself fails to qualify; added a three-sentence paragraph there stating the actual scope plainly (a monad carrying a Lean `Monad`/`LawfulMonad` instance; `Identity` qualifies; `MS S` does not, its `Ret`/`bind` being bare functions with no registered instance), which makes the C4-T1 entry's claim true retroactively — recorded here rather than silently, since it was not true when originally written. F4, the C4-T1 entry's DISCLOSED CHOICES item (1) similarly claims the actor-category substitution (`Set` acting on itself by product, not the `Tens` actor of the Set/Tens/Identity example) is "disclosed as such ... in the blueprint prose"; also not true as landed — added one sentence after `def:type-cat-interpretation` naming the substitution and why it is harmless (`Definition~\ref{def:categorical-interpretation}`'s `act` field carries no coherence law, so the choice is not load-bearing in any of the five items' proofs), again making the earlier claim true retroactively. No `NeSyCat/**` statement or proof term touched (F1 is doc comments only). Gates: `scripts/check.sh` GREEN (whole project, comment-only Lean change), `scripts/sorry-report.sh` 0/0 (unchanged), `scripts/blueprint.sh` GREEN (structure/kind-check/census/classification sentinels all unchanged — no new or re-kinded environment, no `\lean`/`\leanok` mark added or moved; only the step-correspondence sentinel moved, to the derived `envs=5 displays=15 tags=15 names=55` above; CORRESPONDENCE OK). C4-T3 (2026-08-12): **thm:pointwise-eval-kleisli IS MARKED** (`NeSyCat.pointwise_eval_kleisli`, \leanok on statement and proof). The env is proved AS STATED, not over a model: `P.semBatchOn φ h s i = P.semPlain φ h (P.ctxAt i φ.on (s i))` for a batch `s : Fin B → ctxObj P.domB φ.on`, which is the env's own display. Hypothesis bundle `BatchNaturalReading` carries EXACTLY the env's batch-naturality, four laws — funMorK/relMorK/connMor/quanMor, each `batched ⨟ project = project ⨟ unbatched` — and nothing stronger; `funMorB`/`relMorB` carry no law, honestly disclosed, because the grammar has no syntax for passing parameters so no clause reads them. Proof route: lem:kleisli-formula-natural at the InterpretationMorphism that batch-naturality makes of `ev i`, read at the batch's i-th input. NON-VACUITY WITNESS formalized alongside (`witCat`/`witLog`/`witDom`/`witReading`/`witFormula`, `pointwise_eval_kleisli_witness`): the theorem is an implication, so a concrete signature, reading and formula satisfying all four laws at every batch size is exhibited — the hypothesis is SATISFIABLE, not merely unrefuted. CLASSIFIED STRUCTURAL, verified against the full type before the sentinel moved: hypotheses are only `[Monad M] [LawfulMonad M] [DecidableEq sigG.Var]`, no Semiring, no lattice, no order, no categorical hypothesis on the value carrier. So batching is transparent WHATEVER THE WEIGHTS ARE, and this is now the strongest carrier-agnostic statement in the document (6 of 64, up from 5 of 63). Gates: CHECK GREEN 0 errors, 0/0, BLUEPRINT GREEN 128 envs / 127 names, census 807/127/680/0, vacuity 45/0. **AMBER COUNT NOW 1**: only lem:quantifier-nestable-order remains unmarked. V-C4-T3 (blind) PASS_WITH_NOTES — THE MARK IS HONEST, verified unhedged: the verifier traced all 13 BatchNaturalReading fields against Fm.sem/Tm.sem themselves, confirmed the four laws map 1-1 onto InterpretationMorphism's compat fields with NO fifth law and no algebraic hypothesis, confirmed funMorB/relMorB's law-free status by checking Fm.sem's four clauses read only relMorK/connMor/quanMor and Tm.sem's only funMorK, confirmed all four Fm constructors covered, and found no circularity. Witness genuine; classification structural TRUE; axioms triple; zero blast radius by pairwise hashing; exactly one env now unmarked. TWO MAJOR FINDINGS, BOTH LEAD PROSE (the claim that both were FIXED HERE was itself wrong, and V-C4-PROSE caught it: only one of the object-family misdescription's two occurrences was repaired; the theorem's own proof paragraph still carried the discredited version and was fixed later, with the false unsatisfiability claim): (1) content.tex still said the theorem "stays unproved" seventy lines below where the LEAD had marked it \leanok IN THE SAME COMMIT — the same assert-in-two-places failure the LEAD had written a rule about hours earlier; (2) the motivating prose described the object family as "projecting a batch to its i-th component", but the implementation has domMor := 𝟙. **THE LEAD'S DESIGN FRAMING WAS WRONG**: the batch lives ONLY in the monad, not in the domain objects, and the naive batch-the-object reading is UNSATISFIABLE — for |Bidx| ≥ 2 with any relation symbol it forces one truth value to equal every batch slice at once. The dead T3 worker discovered this (commit "Correct where the batch lives"); the LEAD had pinned the opposite into a frontier seat's ticket as the design point. The bundle survived because carrying an object family is harmless when instantiated at the identity. Prose now records where the batch actually lives and why the alternative fails. Also added the six substrate items to the env's \uses so the dependency graph reports the true support. |
