...
| Completions ([0,∞] and [−∞,∞] as first-class truth structures) | proved (C3-CMP, worktree b0b7974 → main): `NeSyCat/LogicalLayer/Completions/{Completions,MassCompletion,LogCompletion}.lean`. Mass completion `ℝ≥0∞` as `BLatCSRng` + `LinBLat2CMon` (`inst:mass-completion-latcsrng`, `inst:mass-completion-linblat2cmon`); the two SEPARATING COUNTEREXAMPLES proved, not asserted: ⊗-amplification kills sub-meet (`thm:mass-submeet-fails`, witness p=q=3, 9≰3) and MIX fails (`thm:mass-mix-fails`, 9≰6), making the attenuating([0,1])/amplifying([0,∞]) dichotomy a proven fact; `thm:mass-completion-not-unitbounds` states UnitBounds as the boundary between REGIMES, not between logic and non-logic; the DM question on [0,∞] RESOLVED IN THE NEGATIVE (`thm:no-dm-mass-completion`), not left open; log completion by transport (`inst:log-completion-latcsrng`); the deferred bounds clauses of the lifted-mass/lifted-log lemmas discharged (`lem:mass-completion-bounds`, `lem:log-completion-bounds`). Worktree-base disclosure: branched at 3b5bce6, so its own gate numbers are worktree-era; the merged main-line sentinels are recorded at the merge commit. |
