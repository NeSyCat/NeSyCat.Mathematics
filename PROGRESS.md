...
- **C3-EXEC-FIX (closing the V-C3EXEC blind-verification FAIL,
  ...
  `batchTransformer`, `ev_isMonadMorphism`, `batch_layer_exact`, all
  `{B : ℕ} {M : Type → Type u} {X Y : Type} [Monad M] [LawfulMonad M]`,
  no algebraic constant anywhere; `ℕ`/`Fin` occur only as batch size and
  ...
  constraint on the VALUE carrier" and still presupposes a lawful
  ambient monad. (2) THE GATE IS NOW A GATE. The section used to end in
