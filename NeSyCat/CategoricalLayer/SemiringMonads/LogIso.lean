...
-- (A1 bijection-law companion of `logTensEquiv`,
...
-- (C2-E4a/A2 completeness census: pre-existing
-- internal helper, not itself blueprint-cited)
@[simp, blueprint_internal] theorem logToFun_zero : logToFun 0 = (⊥ : LogS) := logToFunWB_zero
...
-- (C2-E4a/A2 completeness census: pre-existing
-- internal helper, not itself blueprint-cited)
@[simp, blueprint_internal] theorem lse_bot_left (b : LogS) : lse ⊥ b = b := rfl
-- (C2-E4a/A2 completeness census: pre-existing
-- internal helper, not itself blueprint-cited)
@[simp, blueprint_internal] theorem lse_bot_right (a : ℝ) : lse (a : LogS) ⊥ = (a : LogS) := rfl
...
-- (C2-E4a/A2 completeness census: pre-existing
-- internal helper, not itself blueprint-cited)
@[simp, blueprint_internal] theorem logMul_bot_left (b : LogS) : logMul ⊥ b = ⊥ := rfl
-- (C2-E4a/A2 completeness census: pre-existing
-- internal helper, not itself blueprint-cited)
@[simp, blueprint_internal] theorem logMul_bot_right (a : ℝ) : logMul (a : LogS) ⊥ = ⊥ := rfl
...
-- (C2-E4a/A2 completeness census: pre-existing
-- internal helper, not itself blueprint-cited)
@[simp, blueprint_internal] theorem logS_zero_eq_bot : (0 : LogS) = ⊥ := by
  rw [logS_zero_def, logEquiv_apply, logToFun_zero]

-- (C2-E4a/A2 completeness census: pre-existing
-- internal helper, not itself blueprint-cited)
@[simp, blueprint_internal] theorem logS_one_eq_zero : (1 : LogS) = ((0 : ℝ) : LogS) := by
...
-- (C2-E4a/A2 completeness census: pre-existing
-- internal helper, not itself blueprint-cited)
@[simp, blueprint_internal]
theorem toLogTens_apply (ρ : MS ℝ≥0 X) (x : X) : toLogTens ρ x = logEquiv (ρ x) :=
  Finsupp.mapRange_apply

-- (C2-E4a/A2 completeness census: pre-existing
-- internal helper, not itself blueprint-cited)
@[simp, blueprint_internal]
theorem ofLogTens_apply (a : MS LogS X) (x : X) : ofLogTens a x = logEquiv.symm (a x) :=
  Finsupp.mapRange_apply
