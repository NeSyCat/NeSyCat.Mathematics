...
The non-monotonicity witness (`otimesSq_not_monotone_right`) is the
theorem's core: `p = (1,0)`, `r = (5,0)`, `q = (0,10)`, `r ≤ q` in the
order family but `otimesSq p r = (5,0) ≰ (10,0) = otimesSq p q`. The blueprint's
proof additionally notes `oplus` fails dually (via the cross term in its
...
required content for the theorem's conclusion is the `otimes`/`otimesSq`
direction alone, via `otimesSq_not_lin` (the raw twin of `lem:lin-monotone`:
...
/-- Blueprint `thm:square-not-lin` (`otimesSq` associativity): the seven
cross terms of `otimesSq (otimesSq a b) c`/`otimesSq a (otimesSq b c)`'s slot-`0`
...
/-- Blueprint `thm:square-not-lin` (the core: `otimesSq` is not monotone).
Witness `p = (1,0)`, `r = (5,0)`, `q = (0,10)`: `r ≤ q` in the order family
(slot `1`: `0 ≤ 10`; slot `0` reversed: `5 ≥ 0`), but
`otimesSq p r = (5,0)` and `otimesSq p q = (10,0)`, and `(5,0) ≰ (10,0)` in the
...
theorem otimesSq_not_monotone_right :
  ...
  have e1 : otimesSq (toDual (1 : ℝ≥0), (0 : ℝ≥0)) (toDual (5 : ℝ≥0), (0 : ℝ≥0))
      = (toDual (5 : ℝ≥0), (0 : ℝ≥0)) := by
    unfold otimesSq; simp
  have e2 : otimesSq (toDual (1 : ℝ≥0), (0 : ℝ≥0)) (toDual (0 : ℝ≥0), (10 : ℝ≥0))
...
`otimesSq` does not distribute over `⊔` in its right argument — by
contraposition of `monotone_of_distrib_sup` against the non-monotonicity
witness `otimesSq_not_monotone_right`. Hence, by `lem:lin-monotone`'s
contrapositive, the square rows carry no `LinBLat2Mon` structure: they are
"BLat2Mon-with-DM territory, not Lin". The dual failure of `oplus`
...
theorem otimesSq_not_lin :
    ¬ ∀ p q r : ℝ≥0ᵒᵈ × ℝ≥0, otimesSq p (q ⊔ r) = otimesSq p q ⊔ otimesSq p r := fun hdist =>
  otimesSq_not_monotone_right (monotone_of_distrib_sup _ (hdist (toDual (1 : ℝ≥0), (0 : ℝ≥0))))

/-! ### Persistence in the bounded completion `[0,∞]²` -/

/-- Blueprint `thm:square-not-lin` (persistence witness): `otimesSq`'s raw
...
theorem otimesSqENN_not_monotone_right :
  ...
  have hle : ((toDual (5 : ℝ≥0∞), (0 : ℝ≥0∞)) : ℝ≥0∞ᵒᵈ × ℝ≥0∞) ≤
    ...
    ⟨toDual_le_toDual.mpr (by norm_num), by norm_num⟩
  have hcontra := hmono hle
  have e1 : otimesSqENN (toDual (1 : ℝ≥0∞), (0 : ℝ≥0∞)) (toDual (5 : ℝ≥0∞), (0 : ℝ≥0∞))
      = (toDual (5 : ℝ≥0∞), (0 : ℝ≥0∞)) := by
    unfold otimesSqENN; simp
  have e2 : otimesSqENN (toDual (1 : ℝ≥0∞), (0 : ℝ≥0∞)) (toDual (0 : ℝ≥0∞), (10 : ℝ≥0∞))
      = (toDual (10 : ℝ≥0∞), (0 : ℝ≥0∞)) := by
    unfold otimesSqENN; simp
  rw [e1, e2] at hcontra
  have h1 := hcontra.1
