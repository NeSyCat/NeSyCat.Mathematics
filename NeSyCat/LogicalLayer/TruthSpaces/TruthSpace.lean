...
-- (A1 bijection-law companion of `lift`, content.tex def:lifted-connective)
@[blueprint_internal]
...
-- (A1 bijection-law companion of `twoSlot`, content.tex def:two-slot)
@[simp, blueprint_internal] theorem twoSlot_apply (w : MS S BoolW) : twoSlot w = (w 0, w 1) := rfl

-- (C2-E4a/A2 completeness census: pre-existing
-- internal helper, not itself blueprint-cited)
@[simp, blueprint_internal] theorem twoSlot_symm_apply (p : S × S) :
...
-- (C2-E4a/A2 completeness census: pre-existing
-- internal helper, not itself blueprint-cited)
@[simp, blueprint_internal] theorem distReadoutToFun_coe (d : Dist BoolW) :
...
-- (C2-E4a/A2 completeness census: pre-existing
-- internal helper, not itself blueprint-cited)
@[simp, blueprint_internal] theorem distReadoutInvFun_apply_zero (p : unitInterval) :
    (distReadoutInvFun p).1 0 = ⟨1 - (p : ℝ), by linarith [p.2.2]⟩ := by
  simp [distReadoutInvFun, Finsupp.add_apply, Finsupp.single_eq_same]

-- (C2-E4a/A2 completeness census: pre-existing
-- internal helper, not itself blueprint-cited)
@[simp, blueprint_internal] theorem distReadoutInvFun_apply_one (p : unitInterval) :
...
-- (A1 bijection-law companion of `distReadout`, content.tex def:dist-readout)
@[simp, blueprint_internal] theorem distReadout_apply_coe (d : Dist BoolW) :
