...
theorem twoSlot_otimesM_log (a b : MS LogS BoolW) :
    twoSlot (otimesM a b) =
...
theorem twoSlot_oplusM_log (a b : MS LogS BoolW) :
  ...
  rw [twoSlot_oplusM, logS_mul_eq_logMul, logS_mul_eq_logMul, logS_mul_eq_logMul,
    logS_mul_eq_logMul, logS_add_eq_lse, logS_add_eq_lse]
...
theorem distReadout_otimesD (p q : Dist BoolW) :
    distReadout (otimesD p q) = BLat2Mon.otimes (distReadout p) (distReadout q) := by
  have hval : (otimesD p q).1 1 = p.1 1 * q.1 1 :=
    congrArg Prod.snd (twoSlot_otimesM_mass p.1 q.1)
  apply Subtype.ext
  change ((otimesD p q).1 1 : ℝ) = _
  rw [hval]
  push_cast
  rfl
...
theorem distReadout_oplusD (p q : Dist BoolW) :
  ...
  have hval : (oplusD p q).1 1 = p.1 0 * q.1 1 + p.1 1 * q.1 0 + p.1 1 * q.1 1 :=
    congrArg Prod.snd (twoSlot_oplusM_mass p.1 q.1)
  apply Subtype.ext
  change ((oplusD p q).1 1 : ℝ) = _
  rw [hval, unitInterval.coe_oplus]
